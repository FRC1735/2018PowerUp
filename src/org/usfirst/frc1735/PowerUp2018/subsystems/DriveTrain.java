...
public class DriveTrain extends Subsystem {
    ...
    private final SpeedController leftMotorPractice = RobotMap.driveTrainLeftMotorPractice;
    private final SpeedController rightMotorPractice = RobotMap.driveTrainRightMotorPractice;
    private final DifferentialDrive differentialDrivePractice = RobotMap.driveTrainDifferentialDrivePractice;
    ...
    public void arcadeDrive(double moveValue, double rotateValue, boolean squaredInputs) {
    	...
    	if (m_usePracticeDriveline) {
    		differentialDrivePractice.arcadeDrive(moveValue, rotateValue, squaredInputs);
    	}
    	else {
    ...
    public void tankDrive(double leftValue, double rightValue, boolean squaredInputs) {
    	...
    	// New for 2018, the WPILIB drivetrain is redesigned and assumes positive == forwards so no Y-axis sign shenanigans anymore!
    	if (m_usePracticeDriveline) {
        	differentialDrivePractice.tankDrive(leftValue, rightValue, squaredInputs);
   	}
    	else {
   ...
   public void initDriveTrain() {	   
	   // Get the drivetrain variant we wish to use.
	   // This is just a hack to allow us to run our real robot code (which uses Talons) on our practice bot that uses Victors.
	   // Do call this only once (from the teleop/autonomous init in robot.java) because we will never change the driveline on a physical robot after enabling.
	   // We do it at tele/auto init time (rather than at robot creation init time)
	   // because we want to be able to change the SmartDashboard setting between bootinit and actually enabling the robot.
   		m_usePracticeDriveline = SmartDashboard.getBoolean("UsePracticeDriveline", false); // Default to false if setting not found 

	   		// Turn the safety on for the enabled drivetrain, and off for the disabled one
	   		// This will prevent the unused driveline from throwing timeout "motor safety" errors when it is not being updated...
   		if (m_usePracticeDriveline) {
   			System.out.println("Using Practice Robot driveline...");   			
   			RobotMap.driveTrainDifferentialDrivePractice.setSafetyEnabled(true);
   			RobotMap.driveTrainDifferentialDrive1.setSafetyEnabled(false);
   		}
   		else {
   			System.out.println("Using Real Robot driveline...");
  			RobotMap.driveTrainDifferentialDrive1.setSafetyEnabled(true);
   			RobotMap.driveTrainDifferentialDrivePractice.setSafetyEnabled(false);
   		}
   }
   
   // Member Variables
    boolean m_isInTankMode = true; // true = tank; false = arcade
    boolean m_usePracticeDriveline = false; // true = real robot; false = practice bot
}
